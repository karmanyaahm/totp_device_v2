...
# Karmanyaah Malhotra's driver adapted from the driver by Peter Hinch
...
import sys
DS3231_I2C_ADDR = 104
BADGER_SCL_PIN = machine.Pin(5)
BADGER_SDA_PIN = machine.Pin(4)
...
class DS3231:
    ...
    def get_time(self, set_rtc=False):
        #we don't need that much accuracy for TOTP
    ...
    def save_time(self, tz = 0):
        (YY, MM, mday, hh, mm, ss, wday, yday) = utime.localtime(utime.time() - tz)  # Based on RTC
...
ext_rtc = DS3231(machine.SoftI2C(scl=BADGER_SCL_PIN,
                  sda=BADGER_SDA_PIN,
                  freq=10000))  

if __name__ == 'rtc':
    ext_rtc.get_time(set_rtc = True)
